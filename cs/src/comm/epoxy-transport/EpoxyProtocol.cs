...
namespace Bond.Comm.Epoxy
{
    ...
    internal static class EpoxyProtocol
    {
        ...
        internal struct ClassifyResult
        {
            ...
            /// <summary>
            /// The representation of the message from the <see cref="Frame"/> given to <see cref="Classify"/>. If <c>Classify</c>
            /// was unable to find a payload or returned before it reached the payload, <c>MessageData.data.Array</c> will be
            /// <c>null</c>.
            /// </summary>
        ...
        internal enum ClassifyState
        {
            ...
            // Does the frame have Message data immediately after the EpoxyHeaders (or LayerData if it was present)?
            ExpectMessageData,
            ...
            ref MessageData messageData, ref ProtocolErrorCode? errorCode, Logger logger)
        {
            ...
            messageData.Data = framelet.Contents;
            messageData.IsError = (framelet.Type == FrameletType.ErrorData);
            ...
            ClassifyState state, EpoxyHeaders headers, ref ProtocolErrorCode? errorCode, Logger logger)
        {
            ...
            switch (headers.message_type)
            {
                ...
                default:
                    logger.Site().Warning("Received unrecognized payload type {0}.", headers.message_type);
    ...
    // Struct to hold message data and indicate whether error or not
