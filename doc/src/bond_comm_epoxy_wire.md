...
implement. Payloads (and errors) are marshalled as Compact Binary v1, as this protocol
...
* _**error data**_: opaque blob of data that is the contents of the
  message being sent when the message contains an error
...
| `PayloadData` | 0x54 0x44 ("TD") | Message | One of TD or ER | Compact Binary v1 marshalled | Message payload data |
| `ErrorData` | 0x45 0x52 ("ER") | Message | One of TD or ER | Compact Binary v1 marshalled | Message error data |
...
The EpoxyMessageType indicates what kind of message pattern is being employed:
...
* `Response`: completes the conversation with a response to a previous
  ...
  serialized form of the expected Bond response type for the method invoked or it
  contains an `ErrorData` framelet with a serialized Error.
